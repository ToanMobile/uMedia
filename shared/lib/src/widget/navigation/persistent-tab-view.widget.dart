...
class PersistentTabView extends StatefulWidget {
  PersistentTabView(
      ...
      this.focusFist})
      : super(key: key) {
    assert(items != null || navBarStyle == NavBarStyle.custom, "Items can only be null in case of custom navigation bar style. Please add the items!");
    assert(items == null && itemCount != null || items != null, "In case of custom navigation bar style, the property itemCount is required!");
    ...
    assert(navBarStyle == NavBarStyle.custom || items.length >= 2 && items.length <= 6, "NavBar should have at least 2 or maximum 6 items (Except for styles 15-18)");
    assert(
        navBarStyle == NavBarStyle.custom && customWidget != null || navBarStyle != NavBarStyle.custom, "customWidget shoudl not be null when navBarStyle == NavBarStyle.custom");
  }
...
class _PersistentTabViewState extends State<PersistentTabView> {
                          ...
                          child: Center(
                            child: Container(
                              height: 21.0 +
                                  min(
                                      widget.navBarHeight,
                                      max(widget.decoration.borderRadius.topRight.y ?? 0.0, widget.decoration.borderRadius.topLeft.y ?? 0.0) +
          ...
          tabBar: PersistentBottomNavBar(
            ...
            popAllScreensForTheSelectedTab: (index) {
              if (widget.popAllScreensOnTapOfSelectedTab) {
                ...
                Navigator.popUntil(_contextList[_controller.index], ModalRoute.withName('/9f580fc5-c252-45d0-af25-9429992db112'));
              }
            },
  ...
  Widget build(BuildContext context) {
    _navBarHeight = (widget.resizeToAvoidBottomInset && MediaQuery.of(context).viewInsets.bottom > 0 && widget.hideNavigationBarWhenKeyboardShows)
        ? 0.0
            ...
            : () async {
                if (_controller.index == 0 && !Navigator.canPop(_contextList.first)) {
                  ...
                  if (Navigator.canPop(_contextList[_controller.index])) {
                    ...
                    if (!Navigator.canPop(_contextList[_controller.index]) && _controller.index != 0) {
                      widget.focusFist();
